...
public class LabyrinthDrawer extends JPanel implements Runnable, Observer {
     ...
     * @param mazeView
     */
    public LabyrinthDrawer(MazeModel mazeModel, String mode, MazeView mazeView) {
        this.mazeView = mazeView;
        this.mazeModel = mazeModel;
        this.zoom = mazeView.getTfZoom();
        this.mode = mode;
    ...
    private void buildFrame(){
        ...
        canvas.setPreferredSize(new Dimension(mazeModel.getDimension()*zoom, mazeModel.getDimension()*zoom));
	...
	protected void paintComponent(Graphics g){
        ...
        if(mode.equals("create")) {
            ...
            if(mazeModel.getCellAt(curCoordinate.getX(), curCoordinate.getY()).getVisits() > 1) {
    ...
    public Dimension getPreferredSize() {
        return new Dimension(mazeModel.getDimension()*zoom, mazeModel.getDimension()*zoom);
    }
    
    @Override
    public void run() {
        HashMap<Coordinate, Cell> maze = mazeModel.getMaze();

        if(mode.equals("solve")) {
            curCell = mazeModel.getCellAt(curCoordinate);
            paintComponent(canvas.getGraphics());
        } else {

            for(int i=0; i< mazeModel.getDimension(); i++) {
                for (int j=0; j< mazeModel.getDimension(); j++) {
    ...
    /**
     * Setter method for MazeModel instance var.
     * This also sets the curCoordinate compared to setMazeModel(MazeModel mazeModel)
     * @param mazeModel
     * @param coordinate
     */
    ...
    public void update(Observable observable, Object o) {
        ...
        try {
            Thread.sleep(mazeView.getSpeed());
